...
**Date:** 2026-04-30 (refreshed for cumulative tag — adds Phase 3.x.11.q.y' closure of the v1 honest-scope residuals on top of the 2026-04-30-c baseline; tag moved forward at HEAD)
**Bundle tag reference:** `cumulative-audit-prep-20260430-d` — now covers Phase 3.x.10.y + Phase 3.x.11 + Phase 3.x.11.x + Phase 3.x.11.y + Phase 3.x.11.y.x + Phase 3.x.11.q + Phase 3.x.11.q.y + Phase 3.x.11.q.y' cumulatively. Phase 3.x.11.q.y' closes both v1 honest-scope residuals from the q.y baseline: (1) the `RollbackCacheRequest.n_positions_to_drop` drop-value leak via `always_rollback_k=True` + AES-GCM-encrypted replay-accepted-prefix (rollback-distinct AAD); (2) operator-managed PSK distribution via `X25519AnchoredCipher` (per-request ECDH on `HandoffToken.ephemeral_pubkey` + HKDF over `(request_id, stage_index, chain_total_stages)` → forward secrecy across requests).
