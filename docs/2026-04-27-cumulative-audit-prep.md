...
- **Three regression-discipline rules depend on enforcement mechanism.** R-2026-05-08-1 / -2 / -3 are council commitments. R-2026-05-08-1 was upgraded to **CI-enforced 2026-05-09** at `tests/unit/test_composer_only_invariant.py` (9 tests across 5 tiers: schema-level / Pydantic model / endpoint response / handler source / sibling-tool absence). A future engineer who silently introduces an execute path to `coinbase_offramp_initiate` breaks the test, surfacing the change in code review as "why are you removing the composer-only assertion?" Test docstring documents the supersession-protocol explicitly. R-2026-05-08-2 (Tokenomics ↔ PARTICIPANT_GUIDE coherence) and R-2026-05-08-3 (fleet kill-switch authority preservation) remain council-commitment-only — CI enforcement of those is a follow-on hardening item.
...
- **CI enforcement of R-2026-05-08-2 (Tokenomics ↔ PARTICIPANT_GUIDE coherence) and R-2026-05-08-3 (fleet kill-switch authority preservation).** R-2026-05-08-1 shipped CI enforcement on 2026-05-09 (`tests/unit/test_composer_only_invariant.py`); the other two remain council-commitment-only. CI enforcement of R-2026-05-08-2 would require a check comparing tokenomic claim text across both docs (substring or sentiment-level — non-trivial to encode without false-positives). CI enforcement of R-2026-05-08-3 would require static analysis of any future `FleetKillSwitch.sol` source against the §6.1 schedule. Both deferred to follow-on hardening sprints when the corresponding implementation work begins.
