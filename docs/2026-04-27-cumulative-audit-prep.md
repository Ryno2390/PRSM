...
- **Three regression-discipline rules depend on enforcement mechanism.** R-2026-05-08-1 / -2 / -3 are council commitments, not technical enforcement. Without CI checks pinning the composer-only invariant + tokenomics-coherence freshness check + authority-schedule preservation, the rules depend on founder discipline + auditor review to actually catch breaches. **Auditor: a future hardening sprint should encode at least R-2026-05-08-1 as a CI test (e.g., assert `coinbase_offramp_initiate` schema does NOT include `execute` parameter until a future migration commit explicitly removes the assertion).**
...
- **CI enforcement of the three regression-discipline rules.** Today's CR is a council commitment; technical enforcement (e.g., CI assertion that `coinbase_offramp_initiate` schema does NOT include `execute` until commission-completion ratification supersedes) is deferred. Future hardening sprint.
