...
**Date:** 2026-04-30 (refreshed for cumulative tag — adds Phase 3.x.11.q chain-level Tier C constant-time decorators on top of the 2026-04-29-c baseline; tag moved forward at HEAD)
**Bundle tag reference:** `cumulative-audit-prep-20260430-b` — now covers Phase 3.x.10.y + Phase 3.x.11 + Phase 3.x.11.x + Phase 3.x.11.y + Phase 3.x.11.y.x + Phase 3.x.11.q cumulatively. Phase 3.x.11.q closes the named "Phase 3.x.11.q" structural deferral that has been carrying across four prior phases — Tier C content can now flow through sharded autoregressive decode at Tier A/B perf characteristics via operator-wired chain-level constant-time decorators (M2 single-frame OR M1 cadence-driven yield).
...
> **📌 For auditors starting here:** the most recent audit-prep bundle is **`docs/2026-04-27-cumulative-audit-prep.md`** at tag `cumulative-audit-prep-20260430-b` — it now stacks the original 2026-04-22 economic-layer baseline + 2026-04-27 multi-phase additions + the thirteen streaming-inference phases (§7.1-§7.13 of the cumulative-audit-prep doc). All round-1 review HIGH/MEDIUM findings across all phases are **RESOLVED pre-audit**. The 2026-04-22 baseline (`docs/2026-04-22-phase7.1x-audit-prep.md`) remains authoritative for the 7/7.1/7.1x economic substrate; this refresh ADDS to it rather than replacing. **New for this refresh:** chain-level Tier C decorators (Phase 3.x.11.q) lift the "Tier C structurally denied at the sharded dispatch boundary" line that has been load-bearing across §3.4 + §3.5 + §3.6 of the threat-model addendum — for the executor → caller wire when an operator-wired decorator is in scope. Auditors should read `docs/2026-04-30-phase3.x.11-threat-model-addendum.md` §3.7 (added in 1.3) covering the chain-level vs per-stage scope-honesty point: the chain-executor decorator masks the executor → caller wire ONLY; the per-stage wire continues to dispatch at chain native rate. Phase 3.x.11.q.x is the bundled placeholder for per-stage cadence wrapping. The §7.13 audit-prep section is the auditor-facing summary; the load-bearing trust-policy boundary is the routing-layer's no-silent-fallback invariant — Tier C without a wired decorator surfaces a structured failure naming `make_tier_c_sharded_executor`, not a silent fallback to the leaky path.
