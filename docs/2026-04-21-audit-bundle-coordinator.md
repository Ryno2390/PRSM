...
**Date:** 2026-04-30 (refreshed for cumulative tag — adds Phase 3.x.11.q.x per-stage cadence + M2 response-size padding on top of the 2026-04-30-d baseline; tag moved forward at HEAD; **streaming-inference subsystem roadmap-cap reached — every named structural deferral is now closed**)
**Bundle tag reference:** `cumulative-audit-prep-20260430-e` — now covers Phase 3.x.10.y + Phase 3.x.11 + Phase 3.x.11.x + Phase 3.x.11.y + Phase 3.x.11.y.x + Phase 3.x.11.q + Phase 3.x.11.q.y + Phase 3.x.11.q.y' + Phase 3.x.11.q.x cumulatively. Phase 3.x.11.q.x closes the two named honest-scope items carrying since §7.13: per-stage wire timing leak under sharded autoregressive decode (via `RpcChainExecutor.per_stage_dispatch_cadence_seconds=...`) and M2 response-size leak total joined-text length (via `BatchedTrailingShardedExecutor.pad_to_bytes=...`). Composition: per-stage cadence + chain-level decorators wired together = full-network constant-time masking.
...
> **📌 For auditors starting here:** the most recent audit-prep bundle is **`docs/2026-04-27-cumulative-audit-prep.md`** at tag `cumulative-audit-prep-20260430-e` — it now stacks the original 2026-04-22 economic-layer baseline + 2026-04-27 multi-phase additions + the fifteen streaming-inference phases (§7.1-§7.15, with §7.14.1 covering the q.y' delta and §7.15 covering the q.x roadmap-cap closure). All round-1 review HIGH/MEDIUM findings across all phases are **RESOLVED pre-audit**. The 2026-04-22 baseline (`docs/2026-04-22-phase7.1x-audit-prep.md`) remains authoritative for the 7/7.1/7.1x economic substrate; this refresh ADDS to it rather than replacing. **New for this refresh:** constant-time speculation under Tier C (Phase 3.x.11.q.y) closes two of the three §3.6 wire surfaces (encrypted_proposed_token_probs closes §3.6.2; tail-runner constant_k_commitment closes §3.6.1 wire-shape via verified_token_ids length) and structurally narrows the third (flat_k_mode closes §3.6.3 adaptive-K cross-round correlation). Auditors should read `docs/2026-04-30-phase3.x.11-threat-model-addendum.md` §3.8 (added in 1.4) covering the v1 wire-surface analysis + the load-bearing residual leak: `RollbackCacheRequest.n_positions_to_drop` still encodes `K - accepted_count` on the wire — explicitly NOT closed in v1, with an E2E test (`test_e3_residual_rollback_leak_is_documented`) ASSERTING the leak's wire presence to catch accidental quiet closure as a regression. Closure deferred to Phase 3.x.11.q.y'. The §7.14 audit-prep section is the auditor-facing summary; the load-bearing trust-policy boundary is the routing-layer's `tier_c_speculation_enabled` gate (default-deny on Tier C + temperature > 0; structured failure names the full opt-in contract).
