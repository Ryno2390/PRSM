...
describe("BatchSettlementRegistry", function () {
  ...
  describe("commitBatch", function () {
    it("records a new batch and emits BatchCommitted", async function () {
      ...
      const tx = await registry
        .connect(provider)
        .commitBatch(
          requester.address, DUMMY_ROOT, receiptCount, totalValue, metadataURI
        );
    ...
    it("reverts on zero requester address", async function () {
      await expect(
        registry
          .connect(provider)
          .commitBatch(ethers.ZeroAddress, DUMMY_ROOT, 10, ONE_FTNS, "")
    ...
    it("increments the provider's batch sequence per commit", async function () {
      ...
      await registry
        .connect(provider)
        .commitBatch(requester.address, DUMMY_ROOT, 10, ONE_FTNS, "");
      expect(await registry.providerBatchSequence(provider.address)).to.equal(1);
      await registry
        .connect(provider)
        .commitBatch(requester.address, DUMMY_ROOT_2, 10, ONE_FTNS, "");
    ...
    it("produces distinct batchIds for back-to-back identical commits", async function () {
      const tx1 = await registry
        .connect(provider)
        .commitBatch(requester.address, DUMMY_ROOT, 100, ONE_FTNS, "");
    ...
    it("reverts on zero Merkle root", async function () {
      await expect(
        registry
          .connect(provider)
          .commitBatch(requester.address, ethers.ZeroHash, 10, ONE_FTNS, "")
    ...
    it("reverts on zero receipt count", async function () {
      await expect(
        registry
          .connect(provider)
          .commitBatch(requester.address, DUMMY_ROOT, 0, ONE_FTNS, "")
    ...
    it("accepts zero totalValueFTNS (pathological but not invalid)", async function () {
      await expect(
        registry
          .connect(provider)
          .commitBatch(requester.address, DUMMY_ROOT, 1, 0, "")
      ).to.not.be.reverted;
    });
  });
  ...
  describe("finalizeBatch (with EscrowPool wiring)", function () {
    ...
    beforeEach(async function () {
      const tx = await registry
        .connect(provider)
        .commitBatch(requester.address, DUMMY_ROOT, 100, batchValue, "ipfs://m");
      const r = await tx.wait();
      batchId = r.logs.find(
        (l) => l.fragment && l.fragment.name === "BatchCommitted"
      ).args[0];
    ...
    it("zero-value batch finalizes without invoking escrow pool", async function () {
      ...
      const tx = await registry
        .connect(provider)
        .commitBatch(requester.address, DUMMY_ROOT_2, 1, 0, "");
      const r = await tx.wait();
  ...
  describe("views", function () {
    ...
    beforeEach(async function () {
      const tx = await registry
        .connect(provider)
        .commitBatch(requester.address, DUMMY_ROOT, 100, ONE_FTNS * 10n, "");
      const r = await tx.wait();
