...
| `gpt2-multihost-coldstart-2026-05-25.json` | none (multi-host, cold-start) | **gpt2 2-stage cross-WAN via the production cold-start gossip arc** (sprints 836-845) — Mac probe with NO prior knowledge of operator addresses joins via bootstrap relay, discovers NYC + SFO with relayed `hardware_profile`, allocator splits gpt2's 12 layers as `[(0,6) SFO, (6,12) NYC]`, signed receipt returned in 1.79s. Output `output_hash: fe0663fd…` is **bit-identical to `tier-none-unary-2026-05-22.json`** (the single-host sp688 reference) — proving the multi-host distributed inference produces the same answer as local execution for the same prompt + greedy decode. This is the production milestone receipt for the §7 verifiable-inference promise on a 3-region cold-start-discovered fleet. |
| `gpt2-lambda-a10-coldstart-2026-05-28.json` | none (heterogeneous GPU+CPU fleet) | **gpt2 single-stage on Lambda A10 GPU through cold-start gossip arc** (sprint 836-845). Same Mac probe; fleet now has 3 operators (Lambda A10 + NYC CPU + SFO CPU). Allocator selected the Lambda GPU (cap=11, 33.9 tflops, 22.5 GB VRAM) for single-stage gpt2 dispatch (12 layers easily fit). Signed receipt returned in **124ms — 12× faster than the 2-CPU split** (1.53s) in `gpt2-multihost-coldstart-2026-05-25.json`. Output `output_hash: fe0663fd…` is **STILL bit-identical to the sp688 reference** — proving heterogeneous-fleet inference (GPU vs CPU vs multi-host split) all converge on the same answer for the same prompt + greedy decode. Validates the GPU dispatch path on the production trust stack and closes the 3-tier hardware attestation arc. |
