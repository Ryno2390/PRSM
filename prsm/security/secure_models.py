...
class SecureWebSocketMessage(SecureBaseModel):
    ...
    type: str = Field(..., min_length=1, max_length=50, pattern=r'^[a-zA-Z0-9_-]+$')
    content: Optional[str] = Field(None, max_length=50000)
    conversation_id: Optional[str] = Field(None, pattern=r'^[a-zA-Z0-9_-]+$')
...
class SecureTransferRequest(SecureBaseModel):
    ...
    token_type: str = Field(default="FTNS", pattern=r'^[A-Z]{3,10}$')
...
class SecureFileUpload(SecureBaseModel):
    ...
    checksum: Optional[str] = Field(None, pattern=r'^[a-fA-F0-9]{64}$')  # SHA256
