...
# NWTN Fixed Test Background Runner
# ================================
# Run the NWTN test with the timing bug fix in the background

TIMESTAMP=$(date +%Y%m%d_%H%M%S)
PROGRESS_FILE="nwtn_fixed_progress_${TIMESTAMP}.txt"
LOG_FILE="nwtn_fixed_test_${TIMESTAMP}.log"
PID_FILE="nwtn_fixed_${TIMESTAMP}.pid"

echo "🧠 NWTN Fixed Test Background Runner" | tee "$PROGRESS_FILE"
echo "====================================" | tee -a "$PROGRESS_FILE"
echo "Started: $(date)" | tee -a "$PROGRESS_FILE"
echo "Test: Quantum Gravity Unification (FIXED TIMING)" | tee -a "$PROGRESS_FILE"
...
# Launch the NWTN test with fixed timing
echo "[$(date)] 🧠 Launching NWTN test with FIXED timing calculation..." | tee -a "$PROGRESS_FILE"
nohup python3 test_nwtn_direct_prompt_1.py > "$LOG_FILE" 2>&1 &
TEST_PID=$!
...
echo "[$(date)] 🏁 NWTN test completed with exit code: $EXIT_CODE" | tee -a "$PROGRESS_FILE"

if [ $EXIT_CODE -eq 0 ]; then
    echo "[$(date)] ✅ Test completed successfully" | tee -a "$PROGRESS_FILE"
    echo "[$(date)] 📄 Check results in generated JSON files" | tee -a "$PROGRESS_FILE"
else
    echo "[$(date)] ❌ Test failed - check $LOG_FILE for details" | tee -a "$PROGRESS_FILE"
fi

echo "[$(date)] 📊 Final log summary:" | tee -a "$PROGRESS_FILE"
echo "   - Full log: $LOG_FILE" | tee -a "$PROGRESS_FILE"
echo "   - Progress: $PROGRESS_FILE" | tee -a "$PROGRESS_FILE"
echo "   - Results: Look for nwtn_direct_prompt1_results_*.json" | tee -a "$PROGRESS_FILE"