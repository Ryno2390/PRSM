...
| MED-7 | Remediate | ~1 hr | Setter validation with `code.length > 0` + interface check. | **REMEDIATED-PARTIAL 2026-05-06** (`setEscrowPool` / `setStakeBond` / `setSignatureVerifier` enforce `code.length > 0` for non-zero values; `address(0)` disable mode preserved). 2026-05-07 re-audit (A-07) flagged: ERC165 interface check + 7-day timelock NOT shipped. Per-batch D-03 snapshot still protects in-flight batches; single-batch capture window remains until v2 deploy. Operational defense: Foundation Safe 2-of-3 + 14-day public review per POL-2 §4. |
