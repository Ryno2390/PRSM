...
async def search_models(
    ...
    sort_by: str = Query("popularity", pattern=r'^(popularity|price|created_at|name)$', description="Sort field"),
...
async def get_my_rentals(
    status: Optional[str] = Query(None, pattern=r'^(active|expired|cancelled|suspended)$'),
    current_user: str = Depends(get_current_user)
