...
async def slack_alert_handler(alert: DatabaseAlert):
    """Send database alerts to Slack via webhook URL from settings."""
    import os
    import httpx
    ...
    payload = {
        "attachments": [{
            "color": color,
            "title": f"PRSM Database Alert: {alert.alert_type}",
            "text": alert.message,
            "fields": [
                {"title": "Severity", "value": alert.severity.value, "short": True},
                {"title": "Database", "value": alert.database_name, "short": True},
            ...
            "footer": "PRSM Database Monitoring",
        }]
    }
...
async def email_alert_handler(alert: DatabaseAlert):
    """Send database alerts via email using SMTP settings from environment."""
    import os
    import smtplib
    import asyncio
    ...
    subject = f"[PRSM {alert.severity.value.upper()}] Database Alert: {alert.alert_type}"
    body = f"""PRSM Database Alert Notification
...
This is an automated notification from the PRSM database monitoring system.
"""
