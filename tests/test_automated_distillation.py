...
class TestAutomatedDistillation:
    ...
    @contextmanager
    def _mock_ftns_queries(self):
        """Context manager that mocks FTNSQueries for testing."""
        with patch('prsm.compute.distillation.orchestrator.FTNSQueries') as mock_queries:
            mock_queries.get_user_balance = AsyncMock(return_value={"balance": 10000.0})
            mock_queries.reserve_tokens = AsyncMock(return_value=True)
            mock_queries.transfer_tokens = AsyncMock(return_value=True)
            mock_queries.execute_atomic_deduct = AsyncMock(return_value={"success": True})
            mock_queries.execute_atomic_transfer = AsyncMock(return_value={"success": True})
            yield mock_queries
...
async def run_all_tests():
    ...
    try:
        ...
        with test_suite._mock_ftns_queries():
