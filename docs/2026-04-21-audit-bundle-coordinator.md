...
**Date:** 2026-04-30 (refreshed for cumulative tag — adds Phase 3.x.11.q.y constant-time speculation under Tier C on top of the 2026-04-30-b baseline; tag moved forward at HEAD)
**Bundle tag reference:** `cumulative-audit-prep-20260430-c` — now covers Phase 3.x.10.y + Phase 3.x.11 + Phase 3.x.11.x + Phase 3.x.11.y + Phase 3.x.11.y.x + Phase 3.x.11.q + Phase 3.x.11.q.y cumulatively. Phase 3.x.11.q.y closes the "Speculation + Tier C remains denied" line that was load-bearing in §3.7 of the threat-model addendum — speculation can now flow under Tier C content on the executor → stage wire when the operator wires the full opt-in stack: AES-GCM `encrypted_proposed_token_probs` + tail-runner `constant_k_commitment` + client-side `flat_k_mode` + routing-layer `tier_c_speculation_enabled` gate.
