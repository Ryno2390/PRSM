...
**2026-04-22 update (revised 2026-04-24):** promoted from stub to defense-stack composition analysis at `docs/2026-04-22-r8-defense-stack-composition.md` (**R8-COMP-1**). Formalizes the attack chain as sub-threats, maps each sub-threat against five defense layers (L1 TEE / L2 MPC / L3 fingerprinting / L4 watermarking / L5 CIS-1 silicon *[deferred]*), and specifies minimum-sufficient stacks per adversary class (A-L1 opportunistic through A-L4 vendor-colluding). Identifies L3 + L4 as the unsponsored layers with highest leverage per research dollar (~$400k-$650k to commission both); recommends commissioning now ahead of frontier-lab asks so the answer to "what defenses do you have?" is "actively researching with named partners" rather than "defined as open research." Cross-track integration: R2 contributes L2, R3+R7 contribute 1e coverage, R6 contributes crypto-migration readiness for L3/L4. **CIS-1 (L5) reclassified as deferred research per 2026-04-24 revision** — see R8-COMP-1 §3.5 + `docs/archive/research/README.md`; A-L4 vendor-colluding threat remains unmitigated in the launch roadmap without L5 revival. Combined R2+R3+R7+R8 research program (layers L1-L4 only) scopes at $950k-$1.65M over 6-8 quarters.
...
4. **Custom silicon as an open standard** *(deferred research per 2026-04-24)*. Historical research question: can the Foundation specify a confidential-inference chip architecture — tamper mesh, active shield, power-analysis-hardened crypto, per-chip hardware root, attestation anchored in PRSM's ProvenanceRegistry rather than any single vendor's CA — such that multiple fabs/designers can produce compliant hardware under competitive implementation? See `docs/archive/research/2026-04-21-prsm-cis-1-confidential-inference-silicon.md` (archived) for the in-flight draft. **This sub-question is deferred** per canonical Vision docs — the Foundation does not currently ratify silicon standards, and Prismatica's day-1 business does not include chip design / fabrication. If revived via the §9 ratification path, the question re-enters the research track.
...
**Relationship to the Phase 2 launch UX thesis (see `docs/2026-04-12-phase2-remote-compute-plan.md`).** The launch thesis assumes T3 cloud-arbitrage nodes serve the initial demand. Those operators run H100s they do not own (rented from AWS/GCP/Azure). The data-center operator can always dump VRAM without the PRSM operator's cooperation. Therefore *even with H100 CC*, residual trust sits with the underlying cloud provider, not with PRSM's meganode operator. Frontier labs will factor this in when deciding whether to publish. Without a hardware-vendor-independence layer (historically envisioned as PRSM-CIS-1, now deferred), this residual cloud-provider trust remains a known limitation of PRSM's launch-phase defense stack. Mitigation paths: (a) revive CIS-1 later via the §9 ratification path, (b) rely on multi-vendor diversity + attestation-root diversification as partial mitigation, or (c) accept A-L4 vendor-collusion threats as out-of-scope for launch and focus on A-L1/L2/L3.

**Governance split (historical, 2026-04-19; rescinded 2026-04-24):**

Earlier drafts positioned the Foundation as a confidential-inference silicon standards body. That framing has been withdrawn per the 2026-04-24 CIS-1 revision; the Foundation is no longer a chip-certification body. If CIS-1 is later revived under §9 ratification, a new governance-split scoping will be drafted at that time. Until then, Prismatica's commercial scope does not include silicon design / fabrication — see `Prismatica_Vision.md` §2-3 (founder materials) for the actual six-stream + three-adjacency commercial architecture.
...
**Effort if pursued:** substantial. Sub-questions 1-3 are 1-2 quarters of research each, feasibly done by Foundation + academic partners. Sub-question 4 (custom silicon) is deferred research per 2026-04-24 — see `docs/archive/research/README.md`; would be a 5-10 year arc if revived. Sub-question 5 requires contract work tied to ProvenanceRegistry v2.
