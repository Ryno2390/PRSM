...
class ModelListingDB(BaseDBModel):
    ...
    base_price = Column(SQLDecimal(20, 8), default=0)  # Price in FTNS tokens
    price_per_token = Column(SQLDecimal(20, 8), default=0)
    price_per_request = Column(SQLDecimal(20, 8), default=0)
    price_per_minute = Column(SQLDecimal(20, 8), default=0)
    ...
    # Performance Metrics
    average_response_time = Column(SQLDecimal(10, 3))  # seconds
    tokens_per_second = Column(Integer)
    uptime_percentage = Column(SQLDecimal(5, 2))
    quality_score = Column(SQLDecimal(3, 2))  # 0-10 scale
    
    # Usage Statistics
    total_requests = Column(Integer, default=0)
    total_tokens_processed = Column(Integer, default=0)
    total_revenue = Column(SQLDecimal(20, 8), default=0)
    ...
    popularity_score = Column(SQLDecimal(10, 2), default=0)
...
class RentalAgreementDB(BaseDBModel):
    ...
    # Pricing
    total_cost = Column(SQLDecimal(20, 8), nullable=False)
    hourly_rate = Column(SQLDecimal(20, 8))
...
class MarketplaceOrderDB(BaseDBModel):
    ...
    # Pricing
    subtotal = Column(SQLDecimal(20, 8), nullable=False)
...
class RentalAgreement(BaseModel):
    ...
    # Terms
    rental_type: str = Field(..., regex=r'^(hourly|daily|monthly|per_use|unlimited)$')
    start_time: datetime
    ...
    # Status
    status: str = Field(default="active", regex=r'^(active|expired|cancelled|suspended)$')
    payment_status: str = Field(default="pending", regex=r'^(pending|paid|failed|refunded)$')
...
class MarketplaceOrder(BaseModel):
    ...
    currency: str = Field(default="FTNS", regex=r'^[A-Z]{3,10}$')
    
    # Payment
    payment_method: Optional[str] = Field(None, regex=r'^(ftns_balance|crypto_wallet|credit_card)$')
    payment_status: str = Field(default="pending", regex=r'^(pending|completed|failed|cancelled|refunded)$')
    payment_transaction_id: Optional[str] = None
    payment_completed_at: Optional[datetime] = None
    
    # Status
    order_status: str = Field(default="pending", regex=r'^(pending|confirmed|processing|completed|cancelled)$')
    fulfillment_status: str = Field(default="pending", regex=r'^(pending|processing|fulfilled|failed)$')
...
class RentModelRequest(BaseModel):
    ...
    payment_method: str = Field(default="ftns_balance", regex=r'^(ftns_balance|crypto_wallet)$')
...
class MarketplaceSearchFilters(BaseModel):
    ...
    sort_by: str = Field(default="popularity", regex=r'^(popularity|price|created_at|rating|name)$')
