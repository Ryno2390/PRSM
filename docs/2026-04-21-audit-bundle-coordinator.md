...
**Date:** 2026-04-29 (refreshed for cumulative tag — adds Phase 3.x.11.x pipelining + per-token receipt attestation AND Phase 3.x.11.y speculative decoding on top of the 2026-04-30 baseline; tag moved forward at HEAD)
**Bundle tag reference:** `cumulative-audit-prep-20260429-b` — now covers Phase 3.x.10.y + Phase 3.x.11 + Phase 3.x.11.x + Phase 3.x.11.y cumulatively. Speculative decoding (3.x.11.y) closes the load-bearing compute-level pipelining honest-scope deferral; per-iteration receipt attestation (3.x.11.x) closes the no-per-iteration-cryptographic-commitment gap from the 3.x.11 threat-model addendum §3.2.
...
> **📌 For auditors starting here:** the most recent audit-prep bundle is **`docs/2026-04-27-cumulative-audit-prep.md`** at tag `cumulative-audit-prep-20260429-b` — it now stacks the original 2026-04-22 economic-layer baseline + 2026-04-27 multi-phase additions + the eleven streaming-inference phases (§7.1-§7.11 of the cumulative-audit-prep doc). All round-1 review HIGH/MEDIUM findings across all phases are **RESOLVED pre-audit**. The 2026-04-22 baseline (`docs/2026-04-22-phase7.1x-audit-prep.md`) remains authoritative for the 7/7.1/7.1x economic substrate; this refresh ADDS to it rather than replacing. **New for this refresh:** speculative decoding (Phase 3.x.11.y) adds a per-iteration accept-rate timing surface — auditors should read `docs/2026-04-30-phase3.x.11-threat-model-addendum.md` (§3.5 added in 1.1) alongside `docs/2026-04-22-r3-threat-model.md` when evaluating sharded-mode operators. The threat-model addendum's auditor-reading-path entries 8 + 9 (greedy-only gate + best-effort rollback) are the load-bearing scope-honesty points for the speculative slice.
...
1. **`docs/2026-04-27-cumulative-audit-prep.md`** — most recent; cumulative bundle covering all post-economic-layer phases. Read sections in order:
   ...
   - §7.11 — Speculative Decoding (Phase 3.x.11.y). **NEW. Read alongside the threat-model addendum's §3.5 (added in 1.1 — covers the per-iteration accept-rate timing surface) and the addendum's auditor-reading-path entries 8 + 9 (greedy-only gate + best-effort rollback).**
