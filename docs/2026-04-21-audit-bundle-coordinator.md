...
**Date:** 2026-04-30 (refreshed for cumulative tag — adds Phase 3.x.11 sharded autoregressive decode on top of the same-day 3.x.10.y baseline; tag moved forward at HEAD)
**Bundle tag reference:** `cumulative-audit-prep-20260430` — now covers Phase 3.x.10.y + Phase 3.x.11 cumulatively. Sharded autoregressive decode closes the load-bearing tail-only contract that's accumulated since Phase 3.x.8; each chain stage now runs ONCE PER GENERATED TOKEN with KV-cache state surviving locally between iterations.
...
> **📌 For auditors starting here:** the most recent audit-prep bundle is **`docs/2026-04-27-cumulative-audit-prep.md`** at tag `cumulative-audit-prep-20260430` — it now stacks the original 2026-04-22 economic-layer baseline + 2026-04-27 multi-phase additions + the nine streaming-inference phases (§7.1-§7.9 of the cumulative-audit-prep doc). All round-1 review HIGH/MEDIUM findings across all phases are **RESOLVED pre-audit**. The 2026-04-22 baseline (`docs/2026-04-22-phase7.1x-audit-prep.md`) remains authoritative for the 7/7.1/7.1x economic substrate; this refresh ADDS to it rather than replacing. **New for this refresh:** sharded autoregressive decode introduces a per-token wire timing surface and KV-cache state on stages — auditors should read `docs/2026-04-30-phase3.x.11-threat-model-addendum.md` alongside `docs/2026-04-22-r3-threat-model.md` when evaluating sharded-mode operators (R3 cross-references the addendum in its Related-documents header).
...
1. **`docs/2026-04-27-cumulative-audit-prep.md`** — most recent; cumulative bundle covering all post-economic-layer phases. Read sections in order:
   ...
   - §7.9 — Sharded Autoregressive Decode (Phase 3.x.11). **NEW. Read alongside `docs/2026-04-30-phase3.x.11-threat-model-addendum.md` for the per-token timing surface + KV-cache privacy threat model.**
