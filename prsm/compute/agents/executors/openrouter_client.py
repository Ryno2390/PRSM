...
class OpenRouterClient(BaseModelClient):
    ...
    async def _update_model_availability(self) -> None:
        ...
        try:
            # Use configuration manager for model definitions
            config_manager = get_model_config_manager()
            available_models = await config_manager.get_available_models("openrouter")
            
            # Update internal model cache
            self.models = {model.id: model for model in available_models}
            
            logger.info("Updated OpenRouter model availability", 
