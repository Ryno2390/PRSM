...
**Bundle tag reference:** `cumulative-audit-prep-20260429` — now covers Phase 3.x.10.y + Phase 3.x.11 + Phase 3.x.11.x + Phase 3.x.11.y cumulatively. Speculative decoding (3.x.11.y) closes the load-bearing compute-level pipelining honest-scope deferral; per-iteration receipt attestation (3.x.11.x) closes the no-per-iteration-cryptographic-commitment gap from the 3.x.11 threat-model addendum §3.2.
