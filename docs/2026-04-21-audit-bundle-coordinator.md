...
**Date:** 2026-04-29 (refreshed for cumulative tag — adds Phase 3.x.11.y.x sampling-correct speculation under temperature > 0 on top of the 2026-04-29-b baseline; tag moved forward at HEAD)
**Bundle tag reference:** `cumulative-audit-prep-20260429-c` — now covers Phase 3.x.10.y + Phase 3.x.11 + Phase 3.x.11.x + Phase 3.x.11.y + Phase 3.x.11.y.x cumulatively. Sampling-correct speculation (3.x.11.y.x) closes the load-bearing greedy-only-at-T=0 honest-scope deferral from Phase 3.x.11.y via the Leviathan-2023 §2.2 rejection-sampling correction under Option C.1 residual-sampling approximation. v1 (greedy at T=0) traffic remains bit-identical to Phase 3.x.11.y; v2 (T>0) ships a real Tier-A/B production capability that wasn't available before.
...
> **📌 For auditors starting here:** the most recent audit-prep bundle is **`docs/2026-04-27-cumulative-audit-prep.md`** at tag `cumulative-audit-prep-20260429-c` — it now stacks the original 2026-04-22 economic-layer baseline + 2026-04-27 multi-phase additions + the twelve streaming-inference phases (§7.1-§7.12 of the cumulative-audit-prep doc). All round-1 review HIGH/MEDIUM findings across all phases are **RESOLVED pre-audit**. The 2026-04-22 baseline (`docs/2026-04-22-phase7.1x-audit-prep.md`) remains authoritative for the 7/7.1/7.1x economic substrate; this refresh ADDS to it rather than replacing. **New for this refresh:** sampling-correct speculation (Phase 3.x.11.y.x) lifts the v1 greedy-only gate and ships Leviathan-2023 §2.2 rejection sampling. Auditors should read `docs/2026-04-30-phase3.x.11-threat-model-addendum.md` §3.6 (added in 1.2) covering the new content-correlated wire surfaces — most notably `proposed_token_probs` (K floats per VERIFY round, NEW vs Phase 3.x.11.y) and the adaptive-K cross-round content correlation (v2-only). The §7.12 audit-prep section is the auditor-facing summary; the load-bearing scope-honesty point is the explicit acknowledgement that Option C.1's marginal-equals-target invariant holds EXACTLY only in the degenerate-q regime (q=1.0, pure greedy draft) and APPROXIMATELY under stochastic drafts — pinned numerically by `test_option_c1_drift_under_stochastic_q_documented` for the audit trail.
...
1. **`docs/2026-04-27-cumulative-audit-prep.md`** — most recent; cumulative bundle covering all post-economic-layer phases. Read sections in order:
   ...
   - §7.12 — Sampling-Correct Speculation under Temperature > 0 (Phase 3.x.11.y.x). **NEW. Read alongside the threat-model addendum's §3.6 (added in 1.2 — covers the three new content-correlated wire surfaces: accept-rate channel narrows-but-doesn't-disappear, `proposed_token_probs` ships K floats per VERIFY round, adaptive K cross-round correlation). The §7.12 honest-scope section explicitly documents that Option C.1's marginal-equals-target invariant holds EXACTLY only in the degenerate-q regime (q=1.0); the stochastic-q drift is pinned numerically in `test_option_c1_drift_under_stochastic_q_documented` so a future helper change that broke C.1 determinism would surface in CI.**
