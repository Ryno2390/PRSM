...
class FTNSOracle:
    ...
    def _calculate_weighted_price(self, prices: List[OraclePrice]) -> OraclePrice:
        ...
        total_weight = sum(Decimal(str(p.confidence)) for p in prices)
        
        weighted_usd = sum(p.price_usd * Decimal(str(p.confidence)) for p in prices) / total_weight
        weighted_eth = sum(p.price_eth * Decimal(str(p.confidence)) for p in prices) / total_weight
        weighted_btc = sum(p.price_btc * Decimal(str(p.confidence)) for p in prices) / total_weight
        weighted_volume = sum(p.volume_24h * Decimal(str(p.confidence)) for p in prices) / total_weight
        weighted_mcap = sum(p.market_cap * Decimal(str(p.confidence)) for p in prices) / total_weight
        ...
        return OraclePrice(
            ...
            confidence=float(total_weight) / len(prices)
        )
