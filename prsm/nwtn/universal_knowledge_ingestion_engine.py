...
class DocumentProcessor:
    ...
    async def process_content(self, content_path: str, content_format: ContentFormat) -> ProcessedContent:
        ...
        try:
            ...
            if content_format == ContentFormat.PDF:
                raw_content = await self._process_pdf(content_path)
            elif content_format == ContentFormat.DOCX:
                raw_content = await self._process_docx(content_path)
            elif content_format == ContentFormat.TXT:
                raw_content = await self._process_txt(content_path)
            elif content_format == ContentFormat.HTML:
                raw_content = await self._process_html(content_path)
            elif content_format == ContentFormat.MARKDOWN:
                raw_content = await self._process_markdown(content_path)
            else:
    ...
    async def _process_pdf(self, pdf_path: str) -> str:
        ...
        try:
            ...
            logger.warning("PDF processing failed, using fallback", error=str(e))
            # Fallback: return basic file info
            return f"PDF document: {os.path.basename(pdf_path)} (processing failed: {str(e)})"
...
class UniversalIngestionEngine:
    ...
    async def ingest_content(self, 
                           ...
                           content_formats: Optional[List[ContentFormat]] = None) -> IngestionResult:
        ...
        for path, fmt in zip(content_paths, content_formats):
            if fmt in self.processors:
                task = self._process_single_content(path, fmt)
        ...
        ingestion_result = IngestionResult(
            ...
            failed_items=len(failed_content),
            skipped_items=0,
