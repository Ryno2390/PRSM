...
2. **Sampling-correctness invariant under temperature > 0.** v2 traffic (T > 0) routes to `apply_lm_head_and_sample_batch_with_rejection`. The marginal output distribution exactly equals the verifier's softmax(logits/T) distribution — speculation remains a perf optimization, not a sampling change. **Proof.** The pure-NumPy `rejection_sample_speculation` helper implements Leviathan-2023 §2.2 directly; `test_rejection_sample_speculation.py::test_distribution_convergence_under_many_trials` runs 5000 trials with K=1, target [0.5, 0.3, 0.15, 0.05], q = point mass on token 0; empirical histogram matches target within 0.025 atol. The E2E counterpart at `test_phase3_x_11_y_speculative_e2e.py::TestSpeculativeE2EStochastic::test_v2_speculation_first_emit_matches_softmax_marginal` confirms the wire path through 2 stages preserves the marginal at distilgpt2 + T=0.7 + top_k=50 (TV < 0.35 for N=120).
